...
<div class="grid-container" style="width: 960px;">
    <div class="grid-x">
        <?php
        // get all the categories under the best sellers tax
        $args = array(
            'hierarchical' => 1,
            'show_option_none' => '',
            'hide_empty' => 0,
        ...
        // get all teh products under each and put them in a tabbed group
        if (is_array($all_categories)) {
            foreach ($all_categories as $tab_key => $category) {
                ?>
                <div class="cell large-<?php echo $divs; ?>">
                    <p class="top_heading change_tab" id="tab_button_<?php echo $tab_key; ?>"><?php echo $category->name; ?></p>
                </div>
                <?php
            }
        }
...
<div class="grid-container">
    ...
    // get all teh products under each and put them in a tabbed group
    if (is_array($all_categories)) {
        foreach ($all_categories as $tab_key_new => $category) {
            ?>
            <div class="grid-x grid-padding-x" id="tab_group_<?php echo $tab_key_new; ?>" style="display: none;"><?php
            $tab_key_args = array(
                'posts_per_page' => 4,
                'tax_query' => array(
                    'relation' => 'AND',
            ...
            $products = new WP_Query($tab_key_args);
            while ($products->have_posts()) {
                $products->the_post();
                ?>
                <div class="cell large-3">
                    <div class="product_container" style="background-color:white;padding:20px;justify;margin-bottom:30px;">
                        <img src="<?php echo get_the_post_thumbnail_url($products->post->ID); ?>" class="img-responsive" alt="" style="min-height:158px;"/>
                        <p class="product_title" style="min-height: 90px;"><a href="<?php the_permalink();?>"><?php echo $products->post->post_title; ?></a></p>
                        <?php if ( !is_user_logged_in() ) {?>
                            <p class="product_login">
                                <a href="register">Login For Pricing</a>
                            </p>
                        <?php } else { ?>
                            <p class="product_login" style="margin-top:-25px;font-weight:bold;color: black;">$<?php $product = wc_get_product( $products->post->ID );echo $product->get_price(); ?><br><a href="<?php the_permalink();?>" class="woocommerce.button">BUY NOW</a></p>
                         <?php } ?>
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery('#tab_group_0').show();
        jQuery('#tab_button_0').addClass('isActive');

        jQuery('.change_tab').click(function () {

            // get the new id we are changing
            var new_id;
            new_id = jQuery(this).attr('id');
            new_id = new_id.replace('tab_button_','');

            // remove the class
            jQuery('#tab_button_0').removeClass('isActive');
            jQuery('#tab_button_1').removeClass('isActive');
            jQuery('#tab_button_2').removeClass('isActive');
            jQuery('#tab_button_4').removeClass('isActive');
            jQuery('#tab_button_5').removeClass('isActive');

            // add the class
            jQuery('#tab_button_' + new_id).addClass('isActive');

            // hide the tab
            jQuery('#tab_group_0').hide();
            jQuery('#tab_group_1').hide();
            jQuery('#tab_group_2').hide();
            jQuery('#tab_group_3').hide();
            jQuery('#tab_group_4').hide();
            jQuery('#tab_group_5').hide();

            // open the tab
            jQuery('#tab_group_' + new_id).show();
