...
<div class="grid-container">
    <div class="grid-x">
        <?php
            // loop them out
            if (is_array($posts)) {
                foreach ($posts as $onsale_item) {
                    $product = '';
                    $product = wc_get_product($onsale_item->ID);
                    ?>
                        <div class="large-3 medium-3 small-12 cell" style="padding-bottom:40px;padding-top:20px;">
                            <?php $image = wp_get_attachment_url( get_post_thumbnail_id($onsale_item->ID)); ?>
                            <span class="onsale" style="display:none;">Sale!</span>
                            <p style="text-align: center;margin-left:30px;"><a href="<?php echo get_the_permalink($onsale_item->ID);?>"><img src="<?php echo $image; ?>" alt="<?php echo $product->get_title(); ?>" style="max-width: 200px;"></a></p>
                            <p class="title"><a href="<?php echo get_the_permalink($onsale_item->ID);?>"><?php echo $product->get_title(); ?></p>
                            <p class="sku" style="padding-top:5px;padding-bottom:5px;">SKU: <?php echo $product->get_sku(); ?></p>
                            <p class="price">
                                <span class="was_price" style="color:#999;">
                                    <strike>$<?php echo $product->get_regular_price(); ?></strike>
                                </span>
                                <span class="current_price">$<?php echo $product->get_sale_price(); ?></span>
                            </p>
                            <p class="price">(ex GST)</p>
                            <p style="text-align: center;"><a href="?add-to-cart=<?php echo $onsale_item->ID; ?>" data-quantity="1" class="ionline_addtocart" data-product_id="<?php echo $onsale_item->ID; ?>" data-product_sku="<?php echo $product->get_sku(); ?>" rel="nofollow">Add to cart</a></p>
                        </div>
                    <?php
                }
            }
