		...
		<div class="row">
			<div class="col-lg-8 col-12">
				<div class="property-listing">
					<div class="title">
						<h2 style="font-size: 2rem;"><span class="service_name_jquery"></span> Servicing <?php echo get_term(($_GET['area_id']))->name ?></h2>
						<div class="sorting">
                            <label>Sort By</label>
                    ...
                    // echo 'local_listings:' . count($local_listings_arr); dd($listings_arry);
                    // echo '<br>normal_listings:' . count($listings_arry); dd($listings_arry);
                    ...
                    // echo '<br>$listings:' . count($listings); dd($listings); exit();
                    // echo 'local_listings:' . count($local_listings_arr); dd($local_listings_arr); exit();
                    // echo '<br>normal_listings:' . count($listings_arry); dd($listings_arry); exit();
                    // echo '<br>combined:' . count($combined); dd($combined); exit();

                    // (7) recheck the array to make sure that only listings with the area_id are displayed first
                    foreach($combined as $ckey => $cvalue) {
                        if ($cvalue['meta']['listing_information_group_your_local_area'][0] == $_GET['area_id']) {
                            $first_part[] = $cvalue;
                        } else {
                            $second_part[] = $cvalue;
                        }
			...
			<div class="col-lg-4 col-12">
				...
				<div class="listing-form">
					...
					<!--<form method="get" action="<?php /*echo site_url(); */?>/property-lists/" style="display: none;">
						<div class="form-group">
							<label>Region</label>
						...
						<div class="form-group">
							...
							<?php /*foreach ($regions as $region) { */?>
								<select class="form-control region_selects" name="area_id_<?php /*echo $region->term_id; */?>" id="region_<?php /*echo $region->term_id; */?>" style="display: none;">
									...
									<?php /*$terms = get_terms('region', array('parent' => $region->term_id, 'hide_empty' => false)); */?>
